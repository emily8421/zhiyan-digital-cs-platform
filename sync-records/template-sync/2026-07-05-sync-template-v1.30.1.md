...
- commit：因 PowerShell fallback 参数兼容问题，最终按 `template-sync.json` 清单等价执行 `git checkout FETCH_HEAD -- <sync files>`，并刷新 `ai/doc-standards/00-09` 规范镜像；提交 `0a96747 sync template v1.30.1 from ai-project-template`。
...
- 已处理项：Bootstrap 最新 `scripts/sync-template.sh`；Bootstrap 最新 `scripts/sync-template.ps1`；同步方法论到 v1.30.1；派生边界检查通过；确认 `.github/workflows/project-check.yml` 是派生项目版 workflow。
- 待确认项：是否需要按 v1.30.1 的新增 GitHub 上下文检查要求补充项目维护说明；是否需要继续把 PowerShell `Args` 参数兼容问题回流模板。
...
- 同步脚本问题：旧 `scripts/sync-template.ps1` fallback 内部函数参数名 `Args` 在 Windows PowerShell 5.1 下无法接收 `--commit`，导致 `--commit` 仍走 dry-run；本次先 bootstrap v1.30.1 脚本，再按同步清单等价手工同步完成。
- Prompt / 快捷命令理解问题：无。
- 文档说明不清：无。
- 派生项目专属冲突：无。
