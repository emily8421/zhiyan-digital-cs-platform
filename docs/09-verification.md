...
| 验证项 | 方法 | 通过标准 | 当前状态 |
|---|---|---|---|
| Python 后端 | 启动 FastAPI | 服务可访问健康检查；端口待实现时确认 | 待执行 |
| Node 前端 | 启动 H5 / Console | 浏览器可打开页面；端口待实现时确认 | 待执行 |
| 内存 | 观察 Demo 运行占用 | 低于人工确认的 Demo 上限；当前上限待确认 | 待确认 |
| 显存 / GPU | 检查是否依赖 GPU | Phase1 不依赖 GPU；本机未检测到 GPU 信息 | 待执行 |
| 磁盘 | 检查依赖、Mock 数据和日志占用 | 低于人工确认的 Demo 上限；当前上限待确认 | 待确认 |
| Docker | 检查 Docker 服务 | 可选；不可用时走降级 | 当前记录为不可用 |
| PostgreSQL / pgvector | 启动或连接本地服务 | 可选；Phase1 可降级 | 待确认 |
| Mock 数据 | 加载场景包和样例记录 | 两类场景包均可用 | 待执行 |
...
| 验收项 | 对应 AC | 状态 |
|---|---|---|
| H5 完成 6 类样例问答 / 转人工 | AC-001 | 待执行 |
| 产品型与项目型场景包可切换 | AC-002 | 待执行 |
| Mock 进度查询不接真实系统 | AC-003 | 待执行 |
| 高风险 / 无依据不编造 | AC-004 | 待执行 |
| Web 控制台展示运营列表 | AC-005 | 待执行 |
| 日志和样例数据不含敏感信息 | AC-006 | 待执行 |
| 文档链路可追溯 | AC-007 | 本轮文档生成后待审计 |
...
- React + Vite + TypeScript 依赖安装未验证。
- Python 3.14.3 与 FastAPI 生态兼容性需在 Sprint-1 验证；如存在兼容问题，可能需调整 Python 版本。
- 真实飞书通知、真实业务系统和 LLM 均未纳入 Phase1 验证。

## 8. 待人工确认

1. 是否接受 Phase1 以手工 + API 测试为主。
2. 是否要求在 Phase1 强制加入自动化前端测试。
3. 是否允许调整 Python / Node 版本以匹配依赖兼容性。
