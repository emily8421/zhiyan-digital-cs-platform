...
| 通知 | 飞书机器人 webhook | 员工通知 | Phase2 沙箱 | 适配器骨架已实现（默认 Mock，真实发送待凭据） | `.env.local` / 环境变量 | webhook URL / secret（secret） | 已完成启动前评估与默认 Mock / 显式 sandbox 骨架；待沙箱实发（RG-001，见 `docs/research/2026-07-10-tech-env-evaluation-feishu-sandbox.md`） |
...
| RISK-P2-002 | 飞书真实通知权限 / 回调边界未定 | Sprint-8 沙箱联调 | 影响员工侧触达 | Conditional Go（凭据清单 / 出站通知 / 回调边界已定义，适配器骨架已实现，实发待凭据） | 沙箱联调 + 权限确认 | Sprint-8 / RG-001 | 人工提供沙箱 webhook URL / secret 后完成实发验证；回调另拆任务 |
...
|---|---|---|---|---|---|
| RG-001 | 飞书真实通知 | 沙箱联调通过 + 权限 / 回调边界确认 | `docs/research/2026-07-10-tech-env-evaluation-feishu-sandbox.md` | Conditional Go（2026-07-10；适配器骨架已实现） | 真实实发需人工提供 webhook URL / secret；事件回调另拆任务 |
