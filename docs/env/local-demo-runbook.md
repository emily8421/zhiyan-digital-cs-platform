...
3. 如果用户明确要求“启动 / 帮我启动”，运行 `scripts/start-local-demo.ps1`，并提示脚本会打开 3 个 PowerShell 窗口；如果端口已被占用，先停止占用进程或显式改端口，不要让 Vite 自动漂移。
4. 启动后运行或引导用户运行 `scripts/check-local-demo.ps1`，确认后端、H5、Console 均可访问，且 H5 / Console identity marker 匹配本项目。
...
| 演示点 | 操作 | 期望 |
|---|---|---|
| 双场景包切换（Sprint-7） | H5 / API 切换明烁（产品型）与云栖（项目型）场景包 | 不同场景包加载不同知识 / 规则 / Mock 数据，`scenario_pack_id` 正确区分 |
...
| 标准 Demo Sandbox 进度 | H5 发送「我想查一下 DEMO-ORDER-202607-001 的生产进度」 | `answer_type=mock_business`、`source_ref=demo_erp:order:DEMO-ORDER-202607-001`，不接真实系统 |
...
| Docker 不可用 | 本机 Demo 主链路不依赖 Docker；保持 Mock / 本地临时数据即可。PG / pgvector 可选验证已在 RG-002 通过。 |
