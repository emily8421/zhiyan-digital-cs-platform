...
完整规则读取仍是进入分析、设计、编码、模板维护、PR / 合并闭环或任何状态变更前的强制门禁；快速续接升级为执行任务时，必须先完整读取 `ai/index.md` 及其规则清单一次。

在同一个未中断会话中，若已完成上述完整规则读取，且满足以下全部条件，后续顺序治理步骤可复用已加载规则，不必重复全文读取 `ai/index.md` 列出的全部规则文件：
...
出现以下任一情况时，必须重新读取相关规则；若无法判断影响范围，则回到完整规则读取：规则 / 入口文件被修改或同步，用户切换任务或命令，跨仓库 / 跨角色后规则来源不确定，发生长时间中断或上下文压缩，续接记录与 Git 事实冲突，或即将执行的动作不在已加载规则覆盖范围内。
