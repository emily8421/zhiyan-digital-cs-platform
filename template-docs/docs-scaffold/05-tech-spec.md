...
【撰写提要：列出已确认使用的语言、框架、运行时、数据库、模型、工具和最低版本。候选、默认关闭或预留技术必须标明“未启用”。若项目启用运行时版本锁定（见 `ai/project-rules.md` §2.9），应在表中显式记录锁定运行时、版本、版本声明文件、切换工具和锁定原因。】
...
【撰写提要：若 `ai/project-rules.md` §2.9 启用运行时版本锁定，记录锁定目标、声明文件、切换工具和 CI 校验方式；未启用时写“不适用”。】
