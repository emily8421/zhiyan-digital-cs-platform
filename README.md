...
当前仓库仍处于文档基线阶段，暂不建议直接编码。建议流程：

1. 阅读并确认 `docs/03-prd.md` 的 Phase 路线图和 `docs/08-dev-plan.md` 的 Sprint 拆分。
2. 确认待定项：前端框架、存储降级策略、是否真实发送飞书通知、是否启用 LLM、Docker / PostgreSQL 是否纳入 Phase1。
3. 通过文档审计后，从 `docs/08-dev-plan.md` 的 Sprint-1 开始开发。
...
- 外部 API、飞书真实通知、LLM、Docker 镜像和新依赖的引入都需要人工确认。
